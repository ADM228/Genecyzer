...
void Project::makeDefault() {
    ...
    for (int i = 0; i < testString.length() && globalInstruments.size() < 256; i+= 12){
        globalInstruments.push_back(Instrument());
        globalInstruments.back().setName(testString.substr(i, std::min(static_cast<int>(testString.size()-i), 12)));
